...
import { ChambreDisponible, MenuItem, ReservationDetail, ReservationStatut } from '@/lib/api/types';
...
export function mapChambre(raw: unknown): ChambreDisponible {
  ...
  return {
    ...
    photo_url: resolveMediaUrl(pick(r, 'photo_url', 'photoUrl'), r.photos),
  };
}
...
export function mapReservationDetail(raw: unknown): ReservationDetail {
  ...
  return {
    ...
    chambre: {
      numero: asString(pick(chambreRaw, 'numero')),
      type_chambre: asString(pick(chambreRaw, 'type_chambre', 'typeChambre')),
    },
